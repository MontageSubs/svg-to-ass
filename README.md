...
- **完整颜色 / 描边 / 透明度 / 模糊还原**（默认开启）— 按形状逐组发出 `\1c`（fill）、`\3c` + `\bord`（stroke + 宽度，按 `\fscx` 比例缩放）、`\1a` / `\3a`（透明度，自动反转为 ASS 习惯）、`\blur`（来自 `<feGaussianBlur>`）；正确处理 `<g>` 上的继承属性、`style="..."` 内联样式、`url(#id)` 渐变首个 `<stop>`；同色形状自动合并到同一段，避免多余标签切换。
...
- 含 `<g>` 上的继承属性、`style="..."` 内联样式、`url(#id)` 渐变定义的首个 `<stop>`
